...
def experiment(m):
    ...
    # тут створюється словник gt, кожному значенню f1 відповідає табличне значення gt. При великих значення f1 значення gt майже однакові,
    # тому я привласнюю значенням f1 в діапазонах [11, 16] і [17, 136] та значенням f1 > 136 одне значення gt, і додаю у словник з ключами 11, 17 і 137 відповідно,
    # а значення f1 змінюю на відповідні числа для доступу до значень словника через ключ.
    if 11 <= f1 <= 16: f1 = 11
    if 17 <= f1 <= 136: f1 = 17
    if f1 > 136: f1 = 137
